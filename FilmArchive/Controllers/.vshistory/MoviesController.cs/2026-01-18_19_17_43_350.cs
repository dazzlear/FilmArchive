    ...
    public class MoviesController : Controller
    {
        ...
        public async Task<IActionResult> Edit(int id, Movie movie, IFormFile? imageFile)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    // Handle image logic: keep old one if new one isn't uploaded
                    if (imageFile != null)
                    {
                        movie.ImagePath = await SaveImage(imageFile);
                    }
                    ...
                    {
                        // Retrieve the existing path so it isn't overwritten with null
            ...
            return View(nameof(Index), await _context.Movies.ToListAsync());
        }
    }
}